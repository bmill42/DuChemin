...
"""def rows_similar(row1, row2, headers):
    # Variable Setup.
    threshhold = 7  # Number of entries that must be similar.
    num_matches = 0

    # Set the fields to ignore in the similarity calculation.
    fields_to_ignore = ["stop_measure", "start_measure","composition_number", "phrase_number"]
    #fields_to_ignore += []
    field_indexes_to_ignore = {headers.index(field)
                               for field in fields_to_ignore}
    
    weights = { "cadence_final_tone" : 1., "cadence_kind" : 0.95, "cadence_alter" : 0.9, "cadence_role_cantz" : 0.85, "cadence_role tenz" : 0.8}
    for h in headers:
    	if h not in weights:
    		weights[h] = 0.75
...
def rows_similar(row1, row2, headers):
    # Variable Setup.
    threshhold = int(len(headers) * 0.5)  # Number of entries that must be similar.
    num_matches = 0
    ...
    for i in range(len(row1)):
		for j in range(len(row2)):
			if headers[i][:-1] == headers[j][:-1]:
				...
				if row1[i] == row2[j] and row1[i] != "None" and [row1[i], row2[j]] not in pairs_seen:
					num_matches += 1
					#print threshhold
					#pairs_seen.append([row1[i], row2[j]])
...
def make_options_JSON(csv_input, output_file):
    ...
    pair_headers = ["cadence_kind", "cadence_alter", "cadence_role_cantz", "cadence_role_tenz", "cadence_final_tone"] # hardcoded to use the headers that were paired
    
    option_dict = {header: [] for header in pair_headers}

    for row in data:
		for header in pair_headers:
			option_list = []
			for col_index in range(1,len(headers)):
				if headers[col_index][:len(header)] == header and row[col_index] not in option_list:
